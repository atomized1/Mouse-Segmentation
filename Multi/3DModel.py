...
def main():
    ...
    conv1a = keras.layers.Conv3D(filters=32, kernel_size=(64, 64, 64), activation='relu', padding='same')(input_layer)
    conv1b = keras.layers.Conv3D(filters=32, kernel_size=(64, 64, 64), activation='relu', padding='same')(conv1a)
    pool1 = keras.layers.MaxPool3D(pool_size=(2, 2, 2))(conv1b)
    conv2a = keras.layers.Conv3D(filters=64, kernel_size=(64, 64, 64), activation='relu', padding='same')(pool1)
    conv2b = keras.layers.Conv3D(filters=64, kernel_size=(64, 64, 64), activation='relu', padding='same')(conv2a)
    pool2 = keras.layers.MaxPool3D(pool_size=(2, 2, 2))(conv2b)
    conv3a = keras.layers.Conv3D(filters=96, kernel_size=(64, 64, 64), activation='relu', padding='same')(pool2)
    conv3b = keras.layers.Conv3D(filters=96, kernel_size=(64, 64, 64), activation='relu', padding='same')(conv3a)

    dconv3a = keras.layers.Conv3DTranspose(filters=96, kernel_size=(64, 64, 64), padding='same')(conv3b)
    dconv3b = keras.layers.Conv3DTranspose(filters=96, kernel_size=(64, 64, 64), padding='same')(dconv3a)
    unpool2 = keras.layers.UpSampling3D(size=(2, 2, 2))(dconv3b)
    cat2 = keras.layers.concatenate([conv2b, unpool2])
    dconv2a = keras.layers.Conv3DTranspose(filters=96, kernel_size=(64, 64, 64), padding='same')(cat2)
    dconv2b = keras.layers.Conv3DTranspose(filters=96, kernel_size=(64, 64, 64), padding='same')(dconv2a)
    unpool1 = keras.layers.UpSampling3D(size=(2, 2, 2))(dconv2b)
    cat1 = keras.layers.concatenate([conv1b, unpool1])
    dconv1a = keras.layers.Conv3DTranspose(filters=96, kernel_size=(64, 64, 64), padding='same')(cat1)
    dconv1b = keras.layers.Conv3DTranspose(filters=96, kernel_size=(64, 64, 64), padding='same')(dconv1a)
